...
const scrollToSection = (sectionId) => {
  ...
  return (
    <div className="md:pb-0">
      ...
      <header className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
        ...
        >
          ...
          {/* desktop nav - FULL WIDTH CENTERED */}
          <nav className="hidden md:flex items-center w-full justify-center gap-3 lg:gap-4 mx-auto">
    ...
    {/* MOBILE BOTTOM NAVIGATION - Resized to be more compact */}
  ...
  >
    {navLinks.map(({ label, icon: Icon, section }) => (
      <button
        key={label}
        onClick={() => scrollToSection(section)}
        className="group relative flex flex-col items-center p-1.5 transition-transform duration-300 active:scale-75"
      >
        {/* Icon size reduced from w-5 h-5 to w-4 h-4 */}
        <Icon className="w-4 h-4 text-neutral-400 group-hover:text-orange-500 transition-colors duration-300" />
        
        {/* Font size reduced slightly and margin-top tightened */}
        <span className="text-[9px] mt-0.5 font-medium text-neutral-500 group-hover:text-white transition-colors duration-300">
          {label}
        </span>
      </button>
    ))}
